...
def run_outlook_evaluate_once(db: Session = Depends(get_db_session)):
    try:
        OutlookEvaluationService(db).run_once()
...
@router.post("/cleanup/run-once")
def run_cleanup_once(db: Session = Depends(get_db_session)):
    try:
        result = CleanupService(db).run_once()
        message = f"Cleanup completed. Removed {sum(result.removed.values())} rows."
    except Exception:
